...
async def update_rule(rule_id: int, patch: FirewallRuleUpdate):
    """Update an existing rule's destination, port, protocol, or device."""
    rule = await firewall_service.update_rule(rule_id, patch)
    if rule is None:
        raise HTTPException(
...
@router.get("/counters", response_model=list[FirewallDeviceCounter])
async def get_counters():
    """Return iptables counter totals for each device."""
    return await firewall_service.get_device_counters()
